...
class CMA_Block(nn.Module):
    def __init__(self, in_channel_x, in_channel_mask, hidden_channel, out_channel):
        super(CMA_Block, self).__init__()

        self.conv1 = nn.Conv2d(
            in_channel_x, hidden_channel, kernel_size=1, stride=1, padding=0
        )
        self.conv2 = nn.Conv2d(
            in_channel_mask, hidden_channel, kernel_size=1, stride=1, padding=0
        )
