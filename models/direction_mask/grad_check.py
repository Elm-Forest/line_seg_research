...
class DirectionalMaskGenerator(nn.Module):
    ...
    def forward(self, hough_map):
        ...
        for i in range(0, P, batch_size):
            ...
            distance = (rho_cal - rho_b[:, None]).abs()
            batch_mask = (distance < mask_width).float()
            if B > 1:
...
if __name__ == "__main__":
    ...
    numangle, numrho = 180, 200
    dht = C_dht(numAngle=numangle, numRho=numrho).cuda()
    hough_map = dht(img.cuda())
    dmg = DirectionalMaskGenerator(img.shape[2:], num_angle=numangle, num_rho=numrho).cuda()
    direction_mask = dmg(hough_map)
    visualize_results(img, hough_map, direction_mask)

    dmg.train()
    img = img.cuda().requires_grad_(True)
    hough_map = dht(img).double()
    ...
    print("dmg Grad Check Result: ", test)
